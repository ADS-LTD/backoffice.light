...
/**
 * ComptabilityController Controleur de gestion de la comptabilite
...
class ComptabilityController extends Controller
{
    ...
    public function fetch_data(Request $request, Response $response)
    {
        if($request->ajax())
        {
            if($request->from_date != '' && $request->to_date != '')
            {
                $data = DataTable::table('view_comptability')
                ->whereBetween('trans_date', array($request->from_date, $request->to_date))
                ->get();
            }
            else
            {
                $data = DataTable::table('view_comptability')->orderBy('trans_date', 'desc')->get();
            }
