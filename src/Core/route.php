...
use Digitalis\Core\Controllers\TransactionsController;
use Digitalis\Core\Controllers\ComptabilityController;
use Digitalis\Core\Controllers\FetchController;
...
$app->group(R_CUR_LANG, function (App $app) {
    ...
    $app->group('/transactions', function (App $app) {
        $app->get('', TransactionsController::class . ':index')->setName('transactions.read');
        //$app->get('/print', TransactionsController::class . ':print')->setName('transactions.print');
    });

    $app->group('/comptability', function (App $app) {
        $app->get('', ComptabilityController::class . ':index')->setName('comptability.read');
        $app->get('/fetch_data', comptabilityController::class . ':fetch_data')->setName('comptability.fetch_data');
    });
...
$app->group(R_CUR_LANG, function (App $app) {
    ...
    $app->group('/transactions', function (App $app) {
        $app->post('', TransactionsController::class . ':listtransactions')->setName('transactions.dt');
    });

    $app->group('/comptability', function (App $app) {
        $app->post('', ComptabilityController::class . ':listcomptability')->setName('comptability.dt');
    });
...
$transaction = new MenuItem(Lexique::GetString(CUR_LANG, 'transactions'), null, true, 5, 'code');
$transaction->addChildren(new MenuItem(Lexique::GetString(CUR_LANG, 'Number-of-transactions'), 'transactions.read', false, 0, 'clipboard'));
MenuManager::add($transaction);

$comptability = new MenuItem(Lexique::GetString(CUR_LANG, 'comptability'), null, true, 6, 'balance-scale');
$comptability->addChildren(new MenuItem(Lexique::GetString(CUR_LANG, 'comptability'), 'comptability.read', false, 0, 'area-chart'));
MenuManager::add($comptability);