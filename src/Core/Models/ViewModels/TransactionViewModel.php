...
class TransactionViewModel implements ViewModelInterface
{
	 ...
	 * 
	 * @IME\Required{"errMsg":"Veuillez renseigner l'entreprise qui effectue l'opération"}
	 * @var string
	 */
	public $entreprise;

	/**
	 * l'agence qui réalise l'opération
	 * 
	 * @IME\Required{"errMsg":"Veuillez renseigner l'agence qui effectue l'opération"}
	 * @var string
	 */
	public $agence;
	...
	public function convertToEntity()
	{
		...
		$entreprise = EntrepriseDbAdapter::getById($this->entreprise);
		if ($entreprise) {
			$ntrans->setEntreprise($entreprise);
		}
		return $ntrans;

		$agence = AgenceDbAdapter::getByCode($this->agence);
		if ($agence) {
			$ntrans->setAgence($agence);
		} else {
			ModelState::setMessage('branch', Lexique::GetString(CUR_LANG, 'please-enter-the-cash-reg'));
		}
	}
	...
	public static function buildFromEntity($trans = null)
	{
		...
		if (!is_null($trans)) {
			...
			$vmodel->entreprise = !is_null($trans->getEntreprise()) ? $trans->getEntreprise()->getId() : null;
			$vmodel->agence = !is_null($trans->getAgence()) ? $trans->getAgence()->getCode() : null;
